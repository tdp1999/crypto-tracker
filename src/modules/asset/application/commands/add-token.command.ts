...
import { Token } from '@modules/asset/domain/token.entity';
import { Inject, Injectable } from '@nestjs/common';
import { CommandHandler, ICommandHandler } from '@nestjs/cqrs';
import { z } from 'zod';
import { TokenPrice } from '../../domain/token-price.entity';

export const AddTokenCommandSchema = z.object({ refId: IdSchema });
...
export class AddTokenCommandHandler implements ICommandHandler<AddTokenCommand> {
    ...
    async execute(command: AddTokenCommand): Promise<Id> {
        ...
        const { success, data, error } = AddTokenCommandSchema.safeParse(dto);
        if (!success) throw BadRequestError(error, { layer: ErrorLayer.APPLICATION, remarks: 'Token creation failed' });

        const details = await this.tokenRepository.getTokenDetails(data.refId);

        // Check symbol uniqueness
        const existingTokenBySymbol = await this.tokenRepository.findBySymbol(details.symbol);
